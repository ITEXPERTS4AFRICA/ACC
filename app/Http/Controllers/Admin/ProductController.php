...
class ProductController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        if ($request->hasFile('pdf_datasheet')) $data['pdf_datasheet'] = $request->file('pdf_datasheet')->store('pdfs','public');
        Product::create($data);
    ...
    public function update(Request $request, Product $product) {
        $data = $this->validated($request);
        if ($request->hasFile('image')) { Storage::disk('public')->delete($product->image ?? ''); $data['image'] = $request->file('image')->store('products','public'); }
        if ($request->hasFile('pdf_datasheet')) { Storage::disk('public')->delete($product->pdf_datasheet ?? ''); $data['pdf_datasheet'] = $request->file('pdf_datasheet')->store('pdfs','public'); }
        $product->update($data);

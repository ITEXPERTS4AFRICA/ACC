...
<header x-data="{ open: false }" class="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
    <nav class="max-w-7xl mx-auto px-6 h-[72px] flex items-center justify-between gap-6">
        ...
        <a href="{{ route(app()->getLocale().'.home') }}" class="flex-shrink-0">
            <img src="{{ asset('images/logo-acc.png') }}"
                 alt="Atlantic Cocoa Corporation"
        ...
        $navItems = [
            ...
            ['route' => $locale.'.quality',        'label_fr' => 'Qualité & Certifications','label_en' => 'Quality'],
            ['route' => $locale.'.sustainability', 'label_fr' => 'Durabilité & RSE',     'label_en' => 'Sustainability'],
                ...
                <a href="{{ route($item['route']) }}"
                   class="text-[13px] font-medium transition-colors whitespace-nowrap pb-0.5
                          {{ $active
                              ...
                              : 'text-chocolate hover:text-navy' }}">
    ...
    <div x-show="open" x-transition class="xl:hidden bg-white border-t border-gray-100 px-6 py-5 shadow-lg">
        <ul class="flex flex-col gap-4 mb-5">
            ...
            @endforeach
            <li><a href="{{ route(app()->getLocale().'.news') }}" @click="open=false" class="text-sm text-gray-500 hover:text-navy block">{{ app()->getLocale() === 'fr' ? 'Actualités / Médias' : 'News / Media' }}</a></li>
            <li><a href="{{ route(app()->getLocale().'.partners') }}" @click="open=false" class="text-sm text-gray-500 hover:text-navy block">{{ app()->getLocale() === 'fr' ? 'Partenaires B2B' : 'B2B Partners' }}</a></li>
        </ul>
        <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
            <a href="{{ route('locale.switch', 'fr') }}" class="text-xs font-bold {{ app()->getLocale() === 'fr' ? 'text-navy' : 'text-gray-400' }}">FR</a>
            <span class="text-gray-300">|</span>
            <a href="{{ route('locale.switch', 'en') }}" class="text-xs font-bold {{ app()->getLocale() === 'en' ? 'text-navy' : 'text-gray-400' }}">EN</a>
    ...
    <div class="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        ...
        <div class="lg:col-span-1">
            ...
            <p class="text-white/60 text-sm leading-relaxed max-w-xs">
                Leading industrial cocoa processing in West Africa, delivering global quality standards to the international market through excellence and technical precision.
            </p>
        </div>
        ...
        <div>
            ...
            <ul class="space-y-2.5">
                @php
                $footerLinks = [
                    [route(app()->getLocale().'.about'),          app()->getLocale() === 'fr' ? 'À Propos'       : 'About Us'],
                    [route(app()->getLocale().'.products'),       app()->getLocale() === 'fr' ? 'Nos Produits'   : 'Our Products'],
                    [route(app()->getLocale().'.sustainability'),  app()->getLocale() === 'fr' ? 'Durabilité & RSE':'Sustainability'],
                    ['#', 'Privacy Policy'],
                    ['#', 'Terms of Service'],
                ];
                @endphp
                @foreach($footerLinks as [$href, $label])
                <li><a href="{{ $href }}" class="text-sm text-white/70 hover:text-white transition">{{ $label }}</a></li>
                @endforeach
            </ul>
        </div>
        ...
        <div>
            ...
            <div class="flex gap-3">
                ...
                <a href="#"
                   class="w-9 h-9 rounded-full border border-white/20 flex items-center justify-center hover:border-white/60 hover:bg-white/5 transition">
                    <svg class="w-4 h-4 text-white/60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9"/></svg>
                </a>
            </div>
        </div>
    </div>

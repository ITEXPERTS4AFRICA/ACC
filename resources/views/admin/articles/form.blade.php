        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-gray-200 rounded-sm shadow-sm p-6">
                ...
                <div class="space-y-4">
                    <div class="grid sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tag / Catégorie (FR)</label>
                            <input type="text" name="tag" value="{{ old('tag', $article->tag) }}" placeholder="ex: Certifications"
                                   class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-bordeaux">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tag (EN)</label>
                            <input type="text" name="tag_en" value="{{ old('tag_en', $article->tag_en) }}"
                                   class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-bordeaux">
                        </div>
                    </div>
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Extrait (FR)</label>
                            <textarea name="excerpt" rows="3" class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-bordeaux resize-y">{{ old('excerpt', $article->excerpt) }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Excerpt (EN)</label>
                            <textarea name="excerpt_en" rows="3" class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-bordeaux resize-y">{{ old('excerpt_en', $article->excerpt_en) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-sm shadow-sm p-6" x-data="{ langTab: 'fr' }">
                ...
                <div x-show="langTab === 'fr'">
                    <x-admin.tiptap name="content" :value="old('content', $article->content)" :rows="14"/>
                </div>
                <div x-show="langTab === 'en'" style="display:none">
                    <x-admin.tiptap name="content_en" :value="old('content_en', $article->content_en)" :rows="14"/>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-sm shadow-sm p-6">
                ...
                <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-3 mb-4">
                    ...
                    <div class="relative group aspect-square bg-gray-100 rounded-sm overflow-hidden">
                        ...
                        <form action="{{ route('admin.articles.gallery.delete', [$article, $photo]) }}" method="POST"
                              class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center"
                              onsubmit="return confirm('Supprimer ?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="bg-red-600 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm">✕</button>
                        </form>
                    </div>
                    @endforeach
                </div>
                @endif
                <form action="{{ route('admin.articles.gallery.upload', $article) }}" method="POST" enctype="multipart/form-data"
                      class="border-2 border-dashed border-gray-300 rounded-sm p-4 text-center">
                    @csrf
                    <input type="file" name="photos[]" multiple accept="image/*" class="text-sm text-gray-600 mb-2">
                    <button type="submit" class="btn-primary text-xs py-1.5 px-4">Uploader</button>
                </form>
            </div>
            @endif
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-white border border-gray-200 rounded-sm shadow-sm p-5">
                ...
                <div class="space-y-3 mb-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Date de publication</label>
                ...
                <a href="{{ route('admin.articles.index') }}" class="block text-center text-xs text-gray-400 hover:text-gray-600 mt-3">Annuler</a>
            </div>

            <div class="bg-white border border-gray-200 rounded-sm shadow-sm p-5">
                <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">Image principale</h3>
                @if($article->image)
                <img src="{{ asset('storage/'.$article->image) }}" class="w-full rounded-sm mb-3 object-cover aspect-video">
                @endif

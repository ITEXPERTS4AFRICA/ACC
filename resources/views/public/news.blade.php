...
<section class="relative h-[480px] overflow-hidden bg-dark flex items-center justify-center">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6 w-full">
        <div class="bg-white max-w-sm rounded-sm p-8 shadow-2xl">
            ...
            @if($featured)
            <a href="{{ route(app()->getLocale().'.news.detail', $featured->slug) }}"
    ...
    <section class="relative h-[480px] overflow-hidden bg-dark flex items-center">
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-6 w-full">
            <div class="bg-white max-w-sm rounded-sm p-8 shadow-2xl">
                @if($featured?->tag)
                    <span
                        class="inline-block bg-amber text-white text-[9px] font-bold uppercase tracking-[3px] px-3 py-1 mb-5">{{ $featured->tag }}</span>
                @else
                    <span
                        class="inline-block bg-amber text-white text-[9px] font-bold uppercase tracking-[3px] px-3 py-1 mb-5">Annonce
                        Majeure</span>
                @endif
        ...
        <div class="grid lg:grid-cols-2 gap-6 mb-6">
            ...
            <a href="{{ $a ? route(app()->getLocale().'.news.detail', $a->slug) : '#' }}"
               class="block border border-gray-200 rounded-sm overflow-hidden group hover:shadow-md transition-shadow">
                <div class="aspect-[16/10] bg-gray-100 overflow-hidden">
                    ...
                    @else
                    <img src="{{ $page ? asset("storage/".$page['news']->hero_image) : asset('images/news/card-news-1.png') }}" alt="news card" class="w-full h-full object-cover">
                    @endif
                </div>
                <div class="p-6">
                ...
                {{-- Icône filtre --}}
                <button
                    class="border border-gray-200 rounded-sm p-2.5 text-gray-400 hover:text-navy hover:border-navy transition">
            ...
            <div class="grid lg:grid-cols-2 gap-6 mb-6">
                ...
                <a href="{{ $a ? route(app()->getLocale() . '.news.detail', $a->slug) : '#' }}"
                    ...
                    <div class="aspect-[16/10] bg-gray-100 overflow-hidden">
                        ...
                        @else
                            <div
                                class="w-full h-full bg-gradient-to-br from-slate to-dark flex items-center justify-center text-5xl opacity-20">
                                📊</div>
                        @endif
                    </div>
                    <div class="p-6">
                        <span class="section-tag block mb-3">{{ $a?->tag ?? 'Transformations' }}</span>
                        <h3
                            class="font-playfair text-xl font-bold text-navy leading-tight mb-3 group-hover:text-amber transition-colors">
                            {{ $a?->title_locale ?? 'Traçabilité Totale : Déploiement de notre infrastructure Blockchain' }}
                        </h3>
                        <p class="text-gray-500 text-sm leading-relaxed mb-5">
                            {{ Str::limit($a?->excerpt_locale ?? 'D\'ici 2025, 100% de nos fèves seront tracées par satellite et enregistrées sur un registre décentralisé pour garantir une éthique sans compromis.', 180) }}
                        ...
                        <span
                            class="text-navy text-[10px] font-bold uppercase tracking-widest inline-flex items-center gap-1.5 hover:text-amber transition">
...
=======
            ...
            <div class="flex flex-col gap-6">
                ...
                <a href="{{ $b ? route(app()->getLocale().'.news.detail', $b->slug) : '#' }}"
                ...
                <a href="{{ $c ? route(app()->getLocale().'.news.detail', $c->slug) : '#' }}"
...
>>>>>>> 6dd5f60 (fixed hero-banner by admin)
                    </div>
                </a>
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <a href="{{ $b ? route(app()->getLocale() . '.news.detail', $b->slug) : '#' }}"
                        ...
                        <span class="section-tag block mb-3">{{ $b?->tag ?? 'Durabilité' }}</span>
                        <h3
                            class="font-playfair text-lg font-bold text-navy leading-tight mb-2 group-hover:text-amber transition-colors">
                            {{ $b?->title_locale ?? 'Rapport ESG 2023 : Nos objectifs atteints' }}
                        </h3>
                        <p class="text-gray-500 text-sm leading-relaxed mb-4">
                            {{ Str::limit($b?->excerpt_locale ?? 'Réduction de 15% de l\'empreinte carbone sur l\'ensemble de notre chaîne logistique maritime.', 140) }}
                        ...
                        <span
                            class="inline-block border border-navy text-navy text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-sm group-hover:bg-navy group-hover:text-white transition">
                            {{ app()->getLocale() === 'fr' ? 'Consulter' : 'View' }}
                        </span>
                    </a>
                    ...
                    <a href="{{ $c ? route(app()->getLocale() . '.news.detail', $c->slug) : '#' }}"
                        ...
                        <div class="flex items-start gap-4">
                            <div
                                class="w-10 h-10 rounded-full bg-navy/5 flex items-center justify-center flex-shrink-0 text-lg">
                                🤝</div>
                            <div>
                                <span class="section-tag block mb-1">{{ $c?->tag ?? 'Communauté' }}</span>
                                <h3
                                    class="font-playfair text-base font-bold text-navy leading-tight mb-2 group-hover:text-amber transition-colors">
                                    {{ $c?->title_locale ?? 'Financement de 5 nouveaux centres de santé ruraux dans la région de la Nawa.' }}
                                </h3>
                                <span
            ...
            <div class="grid md:grid-cols-3 gap-6">
                ...
                @foreach($bottom as $i => $art)
                    <a href="{{ route(app()->getLocale() . '.news.detail', $art->slug) }}"
                        ...
                        <h3
                            class="font-playfair font-bold text-navy text-base leading-tight mb-3 group-hover:text-amber transition-colors">
                            {{ $art->title_locale }}</h3>
...
>>>>>>> 6dd5f60 (fixed hero-banner by admin)
                ...
                @for($i = $bottom->count(); $i < 3; $i++)
                    <div class="border border-gray-200 rounded-sm p-6">
                        <span class="section-tag block mb-3">{{ $placeholders[$i]['tag'] }}</span>
                        <h3 class="font-playfair font-bold text-navy text-base leading-tight mb-3">
                            {{ $placeholders[$i]['title'] }}</h3>
                        <p class="text-gray-500 text-sm leading-relaxed">{{ $placeholders[$i]['excerpt'] }}</p>
                    </div>
                @endfor
            </div>

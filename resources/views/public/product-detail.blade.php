...
<section class="section-cream py-24">
    <div class="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16">
        ...
        <div class="aspect-[4/3] bg-surface rounded-sm overflow-hidden card-accent">
            @if($product->image)
            <img src="{{ asset('storage/'.$product->image) }}" alt="{{ $product->name_locale }}" class="w-full h-full object-cover">
            @else
            <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-chocolate/5 to-bordeaux/10">
        ...
        {{-- Détails --}}
        <div>
            <p class="text-text-secondary text-lg leading-relaxed mb-8">{{ $product->description_full_locale ?? $product->description_locale }}</p>

            {{-- Variantes --}}
            @if($product->variants)
            <div class="mb-8">
                <h3 class="font-playfair text-xl font-bold text-chocolate mb-4">{{ app()->getLocale() === 'fr' ? 'Variantes disponibles' : 'Available variants' }}</h3>
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between bg-surface border border-border rounded-sm px-4 py-3">
                        <div>
                            ...
                            <span class="ml-2 text-xs bg-industrial text-white px-2 py-0.5 rounded-sm">{{ $variant['code'] }}</span>
                            @endif
                        </div>
                        @if(isset($variant['badge']))
                        <span class="bg-industrial text-white text-xs px-2 py-1 rounded-sm">{{ $variant['badge'] }}</span>
                        @endif
                    </div>
                    @endforeach
                </div>
            ...
            <div class="mt-8 p-6 bg-surface border border-border rounded-sm card-accent">
                <p class="font-semibold text-chocolate mb-2">{{ app()->getLocale() === 'fr' ? 'Intéressé par ce produit ?' : 'Interested in this product?' }}</p>
                <p class="text-text-secondary text-sm mb-4">{{ app()->getLocale() === 'fr' ? 'Contactez nos équipes pour un devis ou des informations techniques.' : 'Contact our teams for a quote or technical information.' }}</p>
    ...
    <div class="max-w-7xl mx-auto px-6 mt-16">
        <h2 class="font-playfair text-2xl font-bold text-chocolate mb-6">{{ app()->getLocale() === 'fr' ? 'Spécifications Techniques' : 'Technical Specifications' }}</h2>
            ...
            <table class="w-full text-sm">
                <tbody class="divide-y divide-border">
                    ...
                    <tr class="hover:bg-cream transition-colors">
                        <td class="px-6 py-4 font-medium text-chocolate w-1/3">{{ $spec['label'] ?? $spec['key'] ?? '' }}</td>

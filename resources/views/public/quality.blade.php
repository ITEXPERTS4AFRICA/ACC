...
{{-- HERO --}}
<section class="relative h-[460px] flex items-end bg-cover bg-center overflow-hidden" style="background-image:url('{{ $page['quality']->hero_image ?? asset('images/quality/hero-quality.png') }}')">
        ...
        <div class="grid md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="border border-gray-200 rounded-sm p-6">
                <div class="flex items-start gap-4 mb-4">
                    <div class="w-12 h-12 bg-navy/5 rounded-sm flex items-center justify-center text-2xl flex-shrink-0">🛡️</div>
                    <div>
                        <h3 class="font-bold text-navy text-lg">FSSC 22000</h3>
                        <p class="text-gray-500 text-sm mt-1 leading-relaxed">{{ app()->getLocale() === 'fr' ? 'Certification Système de Sécurité Alimentaire garantissant une gestion complète des risques sanitaires sur toute la chaîne de production.' : 'Food Safety System Certification ensuring comprehensive management of safety hazards across the production chain.' }}</p>
                    </div>
                </div>
                <a href="#" class="text-navy text-[10px] font-bold uppercase tracking-widest inline-flex items-center gap-1 hover:text-amber transition">
                    View Compliance <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                </a>
            </div>
            ...
            <div class="border border-gray-200 rounded-sm p-6 grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-gray-500 text-sm leading-relaxed">{{ app()->getLocale() === 'fr' ? 'Nos installations sont auditées selon les 4 piliers : Standards du Travail, Santé & Sécurité, Environnement et Éthique des Affaires.' : 'Our facilities are audited against four key pillars: Labour Standards, Health & Safety, Environment, and Business Ethics.' }}</p>
                ...
                <img src="{{ asset('images/quality/sedex-logo.png') }}" alt="SEDEX Logo" class="w-full opacity-80">
            </div>

            {{-- ISO --}}
            <div class="border border-gray-200 rounded-sm p-6">
                <h3 class="font-bold text-navy text-lg mb-2">ISO Certifications</h3>
                <p class="text-gray-500 text-sm leading-relaxed mb-4">{{ app()->getLocale() === 'fr' ? 'Maintien d\'un management de qualité élevé (ISO 9001) et d\'un management environnemental (ISO 14001) dans nos usines d\'Abidjan et du Cameroun.' : 'Maintaining high-level quality management (ISO 9001) and environmental management (ISO 14001) throughout our Abidjan and Cameroon plants.' }}</p>
            ...
            <div class="border border-gray-200 rounded-sm p-6 bg-navy/2">
                ...
                <p class="text-gray-500 text-sm leading-relaxed mb-4">{{ app()->getLocale() === 'fr' ? 'Respect strict du Règlement européen sur la déforestation. Chaque lot de cacao est géolocalisé et tracé jusqu\'à sa parcelle d\'origine.' : 'Strict adherence to the EU Deforestation Regulation. Every batch of cocoa is geolocated and traced back to its plot of origin.' }}</p>
...
<section class="py-20 bg-surface">
    <div class="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
        ...
        <div class="grid grid-cols-2 gap-3">
            ...
            <div class="aspect-[3/4] bg-gray-100 rounded-sm overflow-hidden mt-6"><img src="{{ asset('images/quality/laboratory-2.png') }}" alt="Laboratory Image" class="w-full h-full object-cover"></img>
        </div>
    </div>
</section>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4">
                            <a href="{{ $cert->pdf ? asset('storage/'.$cert->pdf) : '#' }}"
                               class="font-medium text-navy hover:text-amber transition">{{ $cert->name_locale }}</a>
                        </td>
...
<section class="bg-slate py-14 text-center">
    <div class="max-w-xl mx-auto px-6">
        <h2 class="font-playfair text-2xl font-bold text-white mb-3">Ready for Partnership?</h2>
